...
export async function POST(req: Request) {
  ...
  await supabase.from("maschinenbauer_secret_settings").upsert({ id: 1, resend_api_key: apiKey, updated_at: new Date().toISOString() });
  await supabase.from("maschinenbauer_settings").update({ resend_api_key_masked: masked }).eq("id", 1);
