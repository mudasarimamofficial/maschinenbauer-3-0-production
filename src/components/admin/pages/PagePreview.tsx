...
export function PagePreview({ slug, mode, onModeChange, sections }: Props) {
  ...
  useEffect(() => {
    ...
    if (!win) return;
    win.postMessage({ type: "maschinenbauer_page_builder_preview", page: withSections(sections) }, window.location.origin);
  }, [sections]);
          ...
          <iframe
            ...
            onLoad={() => {
              iframeRef.current?.contentWindow?.postMessage(
                { type: "maschinenbauer_page_builder_preview", page: withSections(sections) },
