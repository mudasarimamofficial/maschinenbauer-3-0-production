...
export function MediaPanel({ supabase }: Props) {
  ...
  const loadUsageIndex = useCallback(async () => {
    const chunks: string[] = [];
    const { data: home } = await supabase.from("maschinenbauer_homepage_content").select("content").eq("id", 1).maybeSingle();
    if (home?.content) chunks.push(JSON.stringify(home.content));

    const { data: drafts } = await supabase.from("maschinenbauer_homepage_content_drafts").select("content").eq("id", 1).maybeSingle();
    if (drafts?.content) chunks.push(JSON.stringify(drafts.content));

    const { data: pages } = await supabase
      .from("maschinenbauer_site_pages")
