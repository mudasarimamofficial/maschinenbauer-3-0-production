...
import { MaschinenbauerConfigPanel } from "@/components/admin/MaschinenbauerConfigPanel";
...
export function AdminPageClient({ initialTab = "builder" }: Props) {
  ...
  return (
    ...
    >
      {tab === "builder" || tab === "settings" || tab === "homepage" || tab === "custom" ? (
        <MaschinenbauerConfigPanel
          supabase={supabase}
          onSignOut={async () => {
            await supabase.auth.signOut();
            setLeads([]);
