...
(function(){
  ...
  function renderPortfolio(data){
    ...
    root.innerHTML = arr(sec.items).map(function(item, idx){
      var media = item.image && item.image.url
        ...
        : '<div class="wi-placeholder wp' + esc(String((idx%5)+1)) + '"><span class="wp-icon">' + esc(item.icon || "ðŸŽ¬") + "</span></div>";
      var body = media +
  ...
  function renderTestimonials(data){
    ...
    if(!root) return;
    root.innerHTML = arr(sec.items).map(function(item){
      return '<div class="review-card"><div class="review-stars"><div class="star"></div><div class="star"></div><div class="star"></div><div class="star"></div><div class="star"></div></div><p class="review-text">' + esc(item.quote) + '</p><div class="review-author"><div class="reviewer-avatar">' + esc(item.avatar || "ðŸ‘¤") + '</div><div><div class="reviewer-name">' + esc(item.author) + '</div><div class="reviewer-info">' + esc(item.role) + '</div><div class="review-platform">' + esc(item.platform || "") + "</div></div></div></div>";
    }).join("");
  }
  ...
  function renderContact(data){
    ...
    if(btn) setText(btn, sec.submitText || "Send Message â†’");
  }
  ...
  function renderMarquee(data){
    ...
    root.innerHTML = items.concat(items).map(function(item){
      return '<span class="marquee-item">' + esc(item) + '</span><span class="marquee-dot"> âœ¦ </span>';
    }).join("");
  }
  ...
  function bindContactForm(content){
    ...
    btn.addEventListener("click", function(){
      ...
      fetch("/api/leads", {
        ...
        btn.disabled = false;
        btn.textContent = contact.submitText || "Send Message â†’";
      });
    });
  }
