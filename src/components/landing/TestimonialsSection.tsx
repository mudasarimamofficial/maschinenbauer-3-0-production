...
export function TestimonialsSection({ section }: Props) {
  ...
  const items: ProofItem[] = blocks
    ...
    .map((block, index) => {
      ...
      return {
        id: block.id,
        name: isRealTestimonial ? asString((content as any).name) || "Maschinenbauer client" : fallbackProofTitle(index),
        role:
          isRealTestimonial
  ...
  return (
    <section className="proof-section" id={section.id} data-reveal>
      <div className="proof-shell">
        ...
        {showStats ? (
          <div className="proof-stats" aria-label="Maschinenbauer proof metrics">
            {stats.map((stat) => (
